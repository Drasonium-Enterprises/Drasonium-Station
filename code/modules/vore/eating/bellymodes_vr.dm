...
/////////////////////////// Make any noise ///////////////////////////
	if(digestion_noise_chance && prob(digestion_noise_chance))
		for(var/mob/M in contents)
			if(M && M.get_preference_value(/datum/client_preference/digestion_noises) == GLOB.PREF_YES)
	...
	if(!LAZYLEN(touchable_mobs))
		...
		if(play_sound)
			for(var/mob/M in hearers(VORE_SOUND_RANGE, get_turf(owner))) //so we don't fill the whole room with the sound effect
				if(!M.get_preference_value(/datum/client_preference/digestion_noises) == GLOB.PREF_YES)
					continue
...
///////////////////// Time to actually process mobs /////////////////////
	...
	for(var/mob/living/L as anything in touchable_mobs)
		if(!istype(L))
			crash_with("Touchable mobs had a nonmob: [L]")
			continue
...
/obj/belly/proc/handle_digestion_death(mob/living/M)
	...
	if(compensation > 0)
		if(isrobot(owner))
			...
			owner.adjustNutrition((nutrition_percent / 100)*25.0*compensation)

/obj/belly/proc/steal_nutrition(mob/living/L)
	if(L.nutrition >= 100)
		var/oldnutrition = (L.nutrition * 0.05)
		L.nutrition = (L.nutrition * 0.95)
		owner.adjustNutrition(oldnutrition)
