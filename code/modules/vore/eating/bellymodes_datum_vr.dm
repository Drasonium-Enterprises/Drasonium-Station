...
/datum/digest_mode/digest/process_mob(obj/belly/B, mob/living/L)
	...
	if(L.stat == DEAD)
		if(L.get_preference_value(/datum/client_preference/digestion_noises) == GLOB.PREF_YES)
			if(!B.fancy_vore)
...
/datum/digest_mode/heal/process_mob(obj/belly/B, mob/living/L)
	...
	if(B.owner.nutrition > 90 && (L.health < L.maxHealth))
		...
		B.owner.adjustNutrition(-2)
		if(L.nutrition <= 400)
			L.adjustNutrition(1)
	else if(B.owner.nutrition > 90 && (L.nutrition <= 400))
		B.owner.adjustNutrition(-1)
		L.adjustNutrition(1)
	if(L.stat != oldstat)
		return list("to_update" = TRUE)
	...
	if(egg_contents.len)
		...
		for(var/atom/movable/C in egg_contents)
			if(isitem(C) && egg_contents.len == 1) //Only egging one item
				...
				B.ownegg.icon_scale = 0.2 * B.ownegg.w_class
			...
			if(isliving(C))
				...
				B.ownegg.icon_scale = 0.25 * B.ownegg.w_class
				B.ownegg.update_transform()
				egg_contents -= M
		...
		B.ownegg.icon_scale = clamp(0.25 * B.ownegg.w_class, 0.25, 1)
		B.ownegg.update_transform()
		if(B.ownegg.w_class > 4)
			B.ownegg.slowdown = B.ownegg.w_class - 4
		B.ownegg = null
